  ...
  //  protected String shipFromVendorPartyId = null;
    ...
    public List<GenericValue> makeAllOrderPaymentInfos(LocalDispatcher dispatcher) {
        ...
        while (i.hasNext()) {
            ...
            allOpPrefs.addAll(inf.makeOrderPaymentInfos(delegator, this, index));
            index = index+1;
    ...
    public static class CartPaymentInfo implements Serializable, Comparable<Object> {
        ...
        public List<GenericValue> makeOrderPaymentInfos(Delegator delegator, ShoppingCart cart, int index) {
            ...
            if (valueObj != null) {
                ...
                if (cart.paymentInfo.size() > 1) {
                	CartPaymentInfo cartInfo=  new CartPaymentInfo();
                	cartInfo = cart.getPaymentInfo(index);
                        Debug.logInfo("________payment method for the order -----------" + cartInfo, module);
                        GenericValue opp = delegator.makeValue("OrderPaymentPreference");
                        opp.set("paymentMethodTypeId", cartInfo.paymentMethodTypeId);
                        opp.set("presentFlag", cartInfo.isPresent? "Y" : "N");
                        opp.set("swipedFlag", cartInfo.isSwiped ? "Y" : "N");
                        opp.set("overflowFlag", cartInfo.overflow ? "Y" : "N");
                        opp.set("paymentMethodId",cartInfo.paymentMethodId);
                        opp.set("finAccountId", cartInfo.finAccountId);
                        opp.set("billingPostalCode",cartInfo.postalCode);
                        opp.set("maxAmount", cartInfo.amount);
                        if (refNum != null) {
