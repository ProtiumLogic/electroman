...
-->
<#escape x as x?xml>
<fo:block text-align="center" margin-top="9pt">
             <fo:inline font-weight="bold">${"Local Purchase Order"}</fo:inline><br/>
	</fo:block>
...
--> 
    <#else>
		...
		<fo:block >
		 ...
		 <fo:inline ></fo:inline> 
				<fo:inline margin="right" font-family="Times New Roman" text-align="right">Supplier's Code : ${vendorParty.partyId}</fo:inline>
        </fo:block>
		<fo:block >
        
			 <fo:inline width="200pt" font-family="Times New Roman" >${"Supplier's Name"} : 
			 ${Static['org.ofbiz.party.party.PartyHelper'].getPartyName(vendorParty)}</fo:inline> 
				...
				<fo:inline text-align="right" ></fo:inline>
        </fo:block>
...
<#if orderHeader.getString("orderTypeId") == "SALES_ORDER" && shipGroups?exists?has_content>
    <fo:block  font-style="italic" font-size="9pt">${uiLabelMap.OrderShipmentInformation}:
...
<#if orderTerms?exists?has_content && orderTerms.size() gt 0>
    <fo:block font-family="Times New Roman">${uiLabelMap.OrderOrderTerms}:</fo:block>
    <#list orderTerms as orderTerm>
        <fo:block text-indent="0.2in" font-family="Times New Roman">
