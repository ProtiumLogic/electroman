          ...
          <td valign='middle'>
            <div class='tabletext'>
              <select name="billToCustomerPartyId"<#if sessionAttributes.orderMode?default("") == "SALES_ORDER"> disabled</#if>>
                <option selected="selected" value="Company">Company - ${Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, "Company", false)?if_exists}</option>
...
<div class="screenlet">
        <div class="screenlet-title-bar">
            <h3>Upload invoiceId List</h3>
        </div>
        <div class="screenlet-body">
            <form method="post" action="<@ofbizUrl>createBulkInvoiceIdList</@ofbizUrl>" enctype="multipart/form-data">
                <div>
    ...
    <div class="screenlet">
        <div class="screenlet-title-bar">
            <h3>Balance Inventory Items</h3>
        </div>
        <div class="screenlet-body">
            <form method="post" action="<@ofbizUrl>balanceInventoryItems</@ofbizUrl>" enctype="multipart/form-data">
                <div>
                  <input type="file" size="40" name="uploadedFile" />
                  <input type="submit" value="${uiLabelMap.CommonUpload}" />
                </div>
