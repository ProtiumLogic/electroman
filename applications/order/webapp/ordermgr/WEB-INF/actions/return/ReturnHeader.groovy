...
if (returnHeader) {
       returnDate = DateFormat.getDateInstance(DateFormat.LONG).format(returnHeader.entryDate);
...
if (returnHeader) {
    ...
    if (party) {
        phoneContactMechList = ContactHelper.getContactMech(party, "PHONE_HOME", "TELECOM_NUMBER", false);
        if (phoneContactMechList) {
            context.telecomNumber = delegator.findOne("TelecomNumber", [contactMechId : EntityUtil.getFirst(phoneContactMechList).contactMechId], true);
        }
    }
}
