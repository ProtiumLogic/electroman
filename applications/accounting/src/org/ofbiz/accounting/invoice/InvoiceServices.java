    ...
    public static Map<String, Object> createInvoiceForOrder(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            for (GenericValue currentValue : billItems) {
                ...
                String supplierPartyId = billFromVendorPartyId;
                List<GenericValue> rebateProductList = new ArrayList<GenericValue>();
    			try {
    				rebateProductList = delegator.findByAnd("SupplierProduct", UtilMisc.toMap("productId", orderItem.get("productId"),"partyId",supplierPartyId));
    			} catch (GenericEntityException e) {
    				e.printStackTrace();
    			}
            	...
            	BigDecimal prodRebate = BigDecimal.ZERO;
    			if (UtilValidate.isNotEmpty(rebateProductList)) {
                	GenericValue rebateProduct = EntityUtil.getFirst(rebateProductList);
                	BigDecimal rebate = rebateProduct.getBigDecimal("rebate");
                	if (UtilValidate.isNotEmpty(rebate)) {
                    	rebateRate = rebateProduct.getBigDecimal("rebate");
                    	prodRebate = rebateProduct.getBigDecimal("rebate");
                    	rebateRate = rebateRate.divide(new BigDecimal(100), 100, ROUNDING);
                	}
                	BigDecimal itemRate = orderItem.getBigDecimal("unitPrice");//.multiply(orderItem.getBigDecimal("quantity"));
                	BigDecimal rebateItemRate = itemRate.multiply(rebateRate);
                	promoRate = itemRate.subtract(rebateItemRate).setScale(invoiceTypeDecimals, ROUNDING);
                	adjustmentRate = rebateItemRate.multiply(orderItem.getBigDecimal("quantity")).setScale(invoiceTypeDecimals, ROUNDING);
    			}
    			GenericValue orderAdjustment = delegator.makeValue("OrderAdjustment");
    			orderAdjustment.put("orderAdjustmentId", delegator.getNextSeqId("OrderAdjustment"));
    			orderAdjustment.put("orderId", orderItem.get("orderId"));
    			orderAdjustment.put("orderAdjustmentTypeId", "SUPPLIER_REBATE");
    			orderAdjustment.put("amount", adjustmentRate.negate());
    			orderAdjustment.set("description", "Supplier Rebate Applied "+prodRebate.shortValueExact()+" %");
    			orderAdjustment.set("createdDate", UtilDateTime.nowTimestamp());
    			delegator.createOrStore(orderAdjustment);
    			List<GenericValue> paymentPrefs = delegator.findByAnd("OrderPaymentPreference",UtilMisc.toMap("orderId", orderItem.get("orderId")));
    			if (UtilValidate.isNotEmpty(paymentPrefs)) {
	    			GenericValue opp = EntityUtil.getFirst(paymentPrefs);
	    			List<GenericValue> payments = delegator.findByAnd("Payment",UtilMisc.toMap("paymentPreferenceId", opp.get("orderPaymentPreferenceId")));
	    			GenericValue payment = EntityUtil.getFirst(payments);
	    			BigDecimal orderAmount = opp.getBigDecimal("maxAmount");
	    			orderAmount = orderAmount.subtract(adjustmentRate);
	    			opp.put("maxAmount", orderAmount);
	    			payment.put("amount", orderAmount);
	    			delegator.store(opp);
	    			delegator.store(payment);
    			}
    			BigDecimal billingAmount = BigDecimal.ZERO;
    			if ((!(promoRate.compareTo(BigDecimal.ZERO)>0)|| (invoiceType.equals("SALES_INVOICE")))) {
    				billingAmount = orderItem.getBigDecimal("unitPrice").setScale(invoiceTypeDecimals, ROUNDING);
    			} else {
    				billingAmount= promoRate ;
    			}
                ...
                for (GenericValue adj : itemAdjustments) {
                    ...
                    if (originalOrderItemQuantity.signum() != 0) {
                        if (adj.get("amount") != null) {
                            ...
                            amount = amount.setScale(invoiceTypeDecimals, ROUNDING);
//                            String supplierPartyId = billFromVendorPartyId;
//                            List<GenericValue> rebateProductList = delegator.findByAnd("SupplierProduct", UtilMisc.toMap("productId", orderItem.get("productId"),"partyId",supplierPartyId));
//                            if (UtilValidate.isNotEmpty(rebateProductList)) {
//                            	rebateProductList = EntityUtil.filterByDate(rebateProductList);
//                            	GenericValue rebateProduct = EntityUtil.getFirst(rebateProductList);
//                            	BigDecimal rebateRate = rebateProduct.getBigDecimal("rebate");
//                            	rebateRate = rebateRate.divide(new BigDecimal(100), 100, ROUNDING);
//                                amount = amount.multiply(rebateRate);
//                                Debug.logInfo("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"+amount+"####"+rebateRate, module);
//
//
//                            }
                        }
    ...
    public static Map sendInvoiceToPrinter (DispatchContext dctx, Map context) {
    	       	Delegator delegator = dctx.getDelegator();
    	        LocalDispatcher dispatcher = dctx.getDispatcher();
    	        ...
    	        Map screenInMap = UtilMisc.toMap("invoiceId", invoiceId,"userLogin",userLogin);
    	        String customPrinterName = UtilProperties.getPropertyValue("general.properties", "electroman.printer.name");//"HP-LaserJet-M1005";//"Generic-CUPS-PDF-Printer";//"HP-LaserJet-M1005";////
    	        Map sendPrintValues = UtilMisc.toMap(
    	            "screenLocation", "component://accounting/widget/AccountingPrintScreens.xml#InvoicePDF",
    	           // "printerName", customPrinterName,
    	            "userLogin", userLogin,
    	        ...
    	        try {
    	        	dispatcher.runSync("createFileFromScreen", sendPrintValues);
    	            Debug.logInfo("Printed Picking Ticket for invoice [" + invoiceId + "].", module);      
    	        } catch (Exception e) {
    	            Debug.logError(e, module);
    	            return ServiceUtil.returnError(e.getMessage());
    	        }             
