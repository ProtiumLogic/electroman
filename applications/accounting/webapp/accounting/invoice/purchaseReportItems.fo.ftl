...
<#escape x as x?xml>
    ...
    <fo:table-header height="14px">
      <fo:table-row border-bottom-style="solid" border-bottom-width="thin" border-bottom-color="black">
        ...
        <fo:table-cell>
         ...
         <fo:table-cell>
          <fo:block font-weight="bold" text-align="right">${"Rebate"}</fo:block>
        </fo:table-cell>
        <fo:table-cell>
    ...
    <fo:table-body font-size="10pt">
        ...
        <#list invoiceItems as invoiceItem>
            ...
            <#if !isItemAdjustment>
            	<#assign supplierProducts = delegator.findByAnd("SupplierProduct", Static["org.ofbiz.base.util.UtilMisc"].toMap("partyId",supplierPartyId,"productId",invoiceItem.productId))>
            	<#assign orderItems = delegator.findByAnd("OrderItem", Static["org.ofbiz.base.util.UtilMisc"].toMap("orderId",orderId,"productId",invoiceItem.productId))>
            	<#if orderItems?has_content>
            		<#assign orderItem = Static["org.ofbiz.entity.util.EntityUtil"].getFirst(orderItems) />
				</#if>
            	
            	<#if supplierProducts?has_content>
						<#assign supplierProductinfo = Static["org.ofbiz.entity.util.EntityUtil"].getFirst(supplierProducts) />
					</#if>
					<#assign rebateAmount =0/>
					 <#if supplierProductinfo?exists>
					 	<#assign rebateAmount = supplierProductinfo.rebate/>
					 </#if> 
					 <#if orderItem?has_content>
					 	<#assign itemPrice = orderItem.getBigDecimal("unitPrice")/>
					 </#if>
                ...
                <fo:table-row height="14px" space-start=".15in">
                    ...
                    <fo:table-cell text-align="right">
                        <fo:block> <#if invoiceItem.quantity?exists><@ofbizCurrency amount=itemPrice?if_exists isoCode=invoice.currencyUomId?if_exists/></#if> </fo:block>
                    </fo:table-cell>
                    <fo:table-cell text-align="right">
                        <fo:block>${rebateAmount}</fo:block>
                    </fo:table-cell>
                    <fo:table-cell text-align="right">
