...
<fo:table table-layout="fixed" width="100%" space-after="0.3in">
   <fo:table-column column-width="3.5in"/>
    ...
    <#elseif billingParty?has_content>
    <fo:block><#if billingParty?has_content>Customer Name : ${Static["org.ofbiz.party.party.PartyHelper"].getPartyName(delegator, billingParty.partyId, false)?if_exists}</#if></fo:block>
    <fo:block>Phone No:${phonenumber? if_exists}</fo:block>
    <#else>
