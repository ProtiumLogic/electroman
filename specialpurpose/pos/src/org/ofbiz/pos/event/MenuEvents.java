...
//                    PaymentEvents.payCredit(pos);
            ...
            } else if ("QUOTE_ADD".equals(lastFunc[0])) {
            	PromoEvents.addQuotetoCart(pos);
            }
    ...
    public static synchronized void addItem(PosScreen pos, AWTEvent event) {
        ...
        ArrayList<String>facilityList = finput.getFacilityList();
        Input input = pos.getInput();
        String[] func = input.getFunction("QTY");
        String value = input.value();
        // no value; just return
        String checkFacility = trans.getProductsFacility(pos);
        if("error".equals(checkFacility)) {
        ...
        if (!trans.isOpen()) {
            ...
            if (productId != null) {
            	Map result = trans.getFacilityInfo(productId,facilityList);
                finput.setStockLevel(result);
                finput.setfacilityProductId(productId);
                try {
                    if (!aggregatedItem) {
                        trans.addItem(productId, quantity);
                        BigDecimal reserveQuantity = trans.getItemQuantity(productId);
                        finput.setReserveQuantity(reserveQuantity);
                    } else {
        ...
        // re-calc tax
        trans.calcTax();
        FacilityInput finput1 = pos.getFacilityInput();
        finput1.setFlInput(true);
      // finput.clearFlInput();
        // refresh the others
        pos.refresh();
    }
    ...
    public static synchronized void changeQty(PosScreen pos) {
        ...
        try {
            ...
            BigDecimal reserveQuantity = quantity;
            finput.setReserveQuantity(reserveQuantity);

            Debug.logInfo("#########################################"+sku +"########"+quantity, module);
    ...
    public static synchronized void changeNextItem (PosScreen pos) {   	
        FacilityInput finput = pos.getFacilityInput();
        finput.setFlInput(false);
        pos.refresh();
    }
    public static synchronized void voidItem(PosScreen pos) {
        PosTransaction trans = PosTransaction.getCurrentTx(pos.getSession());
        FacilityInput finput = pos.getFacilityInput();
        QuoteInput qinput = pos.getQuoteInput();
        String sku = null;
        try {
            sku = getSelectedItem(pos);
        } catch (ArrayIndexOutOfBoundsException e) {
