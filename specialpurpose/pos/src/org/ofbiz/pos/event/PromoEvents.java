...
public class PromoEvents {
    ...
    public static synchronized void addQuotetoCart(PosScreen pos) {
    	PosTransaction trans = PosTransaction.getCurrentTx(pos.getSession());
    	...
    	Debug.logInfo("###################### quoteId"+quoteId, module);
    	if (UtilValidate.isEmpty(quoteId)) {
    		pos.showDialog("dialog/error/exception", "QUOTE ID IS EMPTY.");
    		pos.showPage("pospanel");
    	} else {
    		if (trans.validateQuote(quoteId) && trans.checkQuoteExpireDate(quoteId)) {
    	    	Debug.logInfo(quoteId+" is a valid quote and loading cart from it ", module);

    			trans.loadCartFromQuote(quoteId);
    		} else if (!trans.validateQuote(quoteId)) {
    			pos.showDialog("dialog/error/exception", "INVALID QUOTE ID, ENTER A VALID QUOTE ID.");
    			pos.showPage("pospanel");
    		} else if (!trans.checkQuoteExpireDate(quoteId)) {
    			pos.showDialog("dialog/error/exception", "QUOTE HAS BEEN EXPIRED.");
    			pos.showPage("pospanel");
    		}
    	}
    	qinput.setQuoteInput(false);
    	//qinput.clearQuoteInput();
    	//flInput.setFlInput(true);
    	pos.refresh();
    }
